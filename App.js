...
import MapView, { AnimatedRegion, Marker, Polyline, PROVIDER_GOOGLE } from "./src/components/MapComponents";
...
async function customerPushToken() {
  ...
  try {
    ...
    // Push delivery is optional. Password-verified sign-in returns the OTP
    // privately inside this app when device notifications are unavailable.
    return null;
  }
}
...
function AuthScreen({ onAuthenticated, onPendingApproval, pendingEmail, approvedEmail }) {
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "register") {
        ...
        if (application?.accessToken && application?.user) { onAuthenticated(application); return; }
        if (!['pending', 'submitted', 'approved'].includes(application?.status)) throw new Error("Registration was not accepted.");
        onPendingApproval({ name: name.trim(), email: normalizedEmail, password, phone: phone.trim(), notificationReady: application?.notificationReady !== false });
        return;
      }
      const pushToken = await customerPushToken();
      ...
      else if (error instanceof ApiError && ["ACCOUNT_PENDING_APPROVAL", "CUSTOMER_OTP_REQUIRED"].includes(error.code)) onPendingApproval({ name: name.trim(), email: normalizedEmail, password, phone: phone.trim(), otpCode: error.details?.otpCode });
  ...
  return <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 0 : 24} style={styles.authPage}>
...
function PendingApproval({ applicant, onEnterCode, onBackToLogin, onApproved }) {
  ...
  }, [applicant, onApproved, onBackToLogin]);
  return <View style={styles.approvalPage}><StatusBar style="light" /><View style={styles.approvalGlow} /><Logo /><View style={styles.approvalIcon}><Ionicons name="time-outline" size={46} color={colors.orange} /></View><Text style={styles.approvalTitle}>Account awaiting approval</Text><Text style={styles.approvalText}>Thanks, {applicant?.name || "customer"}. Jixels administration must approve your registration. After approval, your six-digit verification code will appear here. Keep this screen open or sign in later to receive it.</Text>{statusMessage ? <Text style={styles.approvalWarning}>{statusMessage}</Text> : null}<View style={styles.approvalSteps}><View style={styles.approvalStep}><Ionicons name="checkmark-circle" size={21} color={colors.green} /><Text style={styles.approvalStepText}>Registration received</Text></View><View style={styles.approvalLine} /><View style={styles.approvalStep}><Ionicons name="hourglass-outline" size={21} color={colors.orange} /><Text style={styles.approvalStepText}>Admin verification and approval</Text></View><View style={styles.approvalLine} /><View style={styles.approvalStep}><Ionicons name="chatbox-ellipses-outline" size={21} color={colors.white} /><Text style={styles.approvalStepText}>Verification code after approval</Text></View></View><Pressable onPress={onEnterCode} style={styles.approvalButton}><Text style={styles.primaryButtonText}>Enter approval code</Text><Ionicons name="keypad-outline" size={18} color={colors.white} /></Pressable><Pressable onPress={onBackToLogin} style={styles.approvalSecondary}><Text style={styles.approvalSecondaryText}>Back to login</Text></Pressable></View>;
}

function ApprovalNotification({ applicant, code, onEnterCode, onBack }) {
  return <View style={styles.approvalPage}><StatusBar style="light" /><View style={styles.approvalGlow} /><Logo /><View style={styles.approvalNoticeCard}><View style={styles.approvalNoticeIcon}><Ionicons name="notifications" size={32} color={colors.white} /></View><Text style={styles.approvalNoticeTitle}>Your account has been approved</Text><Text style={styles.approvalNoticeText}>Your account has been approved. Your one-time verification code is</Text><View style={styles.approvalCode}><Text style={styles.approvalCodeText}>{code}</Text></View><Text style={styles.approvalNoticeHint}>Do not share or expose this OTP. Use it to finish verification. It expires in five minutes.</Text><Pressable onPress={onEnterCode} style={styles.approvalButton}><Text style={styles.primaryButtonText}>Enter verification code</Text><Ionicons name="shield-checkmark-outline" size={18} color={colors.white} /></Pressable><Pressable onPress={onBack} style={styles.approvalSecondary}><Text style={styles.approvalSecondaryText}>Back to login</Text></Pressable></View></View>;
}
...
function OtpVerification({ applicant, onVerified, onBack, initialGate = false, notificationCode = "" }) {
  ...
  return <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.otpPage}>
    ...
    <Animated.View style={[styles.otpCard, { opacity: entrance, transform: [{ translateY: entrance.interpolate({ inputRange: [0, 1], outputRange: [48, 0] }) }, { scale: entrance.interpolate({ inputRange: [0, 1], outputRange: [.94, 1] }) }] }]}> 
      ...
      <Text style={styles.otpResendText}>Your code appears on the approval screen. If it expires, go back to request a new code.</Text>
...
function UnassignedDashboard({ profile, navigate, onRefresh }) {
  return <ScrollView style={styles.pageScroll} contentContainerStyle={styles.pageContent} refreshControl={<PullToRefresh onRefresh={onRefresh} />}>
    <View style={styles.welcomeCard}><View style={styles.welcomeIcon}><Ionicons name="person" size={23} color={colors.white} /></View><View style={{ flex: 1 }}><Text style={styles.eyebrow}>WELCOME BACK</Text><Text style={styles.welcomeName}>{profile.name}</Text><Text style={styles.welcomeCopy}>Your Jixels account is ready. Your vehicle will appear after your agent completes onboarding.</Text></View></View>
    <View style={styles.statGrid}><View style={styles.statCard}><View style={[styles.statIcon, { backgroundColor: colors.bluePale }]}><Ionicons name="car-sport-outline" size={22} color={colors.blue} /></View><Text style={styles.statValue}>0</Text><Text style={styles.statLabel}>My vehicles</Text></View><View style={styles.statCard}><View style={[styles.statIcon, { backgroundColor: "#EAF9F2" }]}><Ionicons name="radio-outline" size={22} color={colors.green} /></View><Text style={styles.statValue}>—</Text><Text style={styles.statLabel}>Tracker status</Text></View></View>
    <View style={styles.infoCallout}><Ionicons name="hourglass-outline" size={21} color={colors.blue} /><Text style={styles.infoCalloutText}>No vehicle or tracker is assigned yet. Your agent enters the vehicle plate during customer onboarding. Pull down to refresh after installation.</Text></View>
    <SectionTitle title="Customer features" />
    <View style={styles.card}><Text style={styles.cardTitle}>What you can do here</Text><Text style={styles.cardSub}>Once your tracker is assigned, this app shows live tracking, vehicle security, M-Pesa payments, receipts, alerts, and reports.</Text><View style={styles.cardActions}><Pressable onPress={() => navigate("alerts")} style={styles.outlineButton}><Ionicons name="notifications-outline" size={17} color={colors.blue} /><Text style={styles.outlineButtonText}>View alerts</Text></Pressable><Pressable onPress={() => navigate("settings")} style={styles.smallPrimary}><Ionicons name="person-outline" size={17} color={colors.white} /><Text style={styles.smallPrimaryText}>My profile</Text></Pressable></View></View>
  </ScrollView>;
}
...
function TrackingScreen({ selectedBike, onSelectBike, accessToken, vehicles = [] }) {
  ...
  if (!bike?.location) {
    // Do not mount the native map until the tracker has supplied a valid GPS
    // point. Some Android devices close the activity when Google Maps starts
    // with a placeholder point and no configured map tiles.
    return <View style={[styles.mapContainer, { backgroundColor: colors.surface, padding: 18, justifyContent: "center" }]}><View style={[styles.card, { gap: 13 }]}><View style={styles.waitingTrackerIcon}><Ionicons name="location-outline" size={26} color={colors.blue} /></View><Text style={styles.waitingTrackerTitle}>Live map is ready</Text><Text style={styles.waitingTrackerText}>{error ?? `${selectedBike.registration} is linked to your account. The live map will open automatically when the tracker sends its first valid GPS location.`}</Text><View style={styles.financeStrip}><View><Text style={styles.microLabel}>VEHICLE</Text><Text style={styles.cardTitle}>{selectedBike.registration}</Text></View><View><Text style={styles.microLabel}>TRACKER</Text><Text style={styles.cardTitle}>{selectedBike.tracker || "Assigned"}</Text></View></View><Pressable onPress={refreshLocation} style={styles.primaryButton}><Ionicons name="refresh" size={18} color={colors.white} /><Text style={styles.primaryButtonText}>{refreshing ? "Checking tracker…" : "Refresh live location"}</Text></Pressable><Pressable onPress={locatePhone} style={styles.outlineButton}><Ionicons name="navigate-outline" size={18} color={colors.blue} /><Text style={styles.outlineButtonText}>Show my phone location</Text></Pressable></View></View>;
    /*
  ...
  const openExternalMap = target => {
    const point = target?.location;
    if (!point) return Alert.alert("Location unavailable", "This tracker has not sent a valid GPS position yet.");
    Linking.openURL(`https://www.openstreetmap.org/?mlat=${encodeURIComponent(point.latitude)}&mlon=${encodeURIComponent(point.longitude)}#map=16/${encodeURIComponent(point.latitude)}/${encodeURIComponent(point.longitude)}`).catch(() => Alert.alert("Map unavailable", "Install or update a browser or Maps app, then try again."));
  };
  const locatedVehicles = allVehicles.filter(vehicle => Number.isFinite(vehicle.location?.latitude) && Number.isFinite(vehicle.location?.longitude));
  return <View style={[styles.mapContainer, { backgroundColor: colors.surface }]}><ScrollView contentContainerStyle={[styles.pageContent, { paddingTop: 18 }]}><View style={[styles.welcomeCard, { backgroundColor: colors.blueDark }]}><View style={styles.welcomeIcon}><Ionicons name="location" size={25} color={colors.white}/></View><View style={{ flex: 1 }}><Text style={styles.welcomeName}>Live tracker locations</Text><Text style={styles.welcomeCopy}>{locatedVehicles.length ? `${locatedVehicles.length} tracker${locatedVehicles.length === 1 ? "" : "s"} reporting a location.` : "Waiting for tracker GPS locations."}</Text></View></View><View style={styles.card}><Text style={styles.sectionTitle}>Your trackers</Text>{allVehicles.map(vehicle => <View key={vehicle.id} style={styles.financeStrip}><View style={{ flex: 1 }}><Text style={styles.cardTitle}>{vehicle.registration}</Text><Text style={styles.cardSub}>{vehicle.model} · Tracker {vehicle.tracker || "Assigned"}</Text><Text style={styles.cardSub}>{vehicle.location ? `${Number(vehicle.location.latitude).toFixed(5)}, ${Number(vehicle.location.longitude).toFixed(5)}` : "No GPS location received yet"}</Text></View><Pressable onPress={() => openExternalMap(vehicle)} style={styles.smallPrimary}><Ionicons name="map-outline" size={17} color={colors.white}/><Text style={styles.smallPrimaryText}>Map</Text></Pressable></View>)}</View><Pressable onPress={refreshLocation} style={styles.primaryButton}><Ionicons name="refresh" size={18} color={colors.white}/><Text style={styles.primaryButtonText}>{refreshing ? "Refreshing locations…" : "Refresh all tracker locations"}</Text></Pressable></ScrollView></View>;
  /* Legacy native map controls retained only for source history.
    <View style={styles.vehiclePicker}><Pressable onPress={() => setVehiclePickerOpen(open => !open)} style={styles.vehiclePickerInput}><Ionicons name="search" size={18} color={colors.blue} /><TextInput value={vehicleSearch} onChangeText={text => { setVehicleSearch(text); setVehiclePickerOpen(true); }} onFocus={() => { setVehicleSearch(""); setVehiclePickerOpen(true); }} placeholder="Search vehicle or plate" placeholderTextColor="#7890A8" style={styles.vehicleSearchInput} /><Ionicons name={vehiclePickerOpen ? "chevron-up" : "chevron-down"} size={17} color={colors.gray} /></Pressable>{vehiclePickerOpen && <View style={styles.vehicleDropdown}>{matchingVehicles.length ? matchingVehicles.map(vehicle => <Pressable key={vehicle.id} onPress={() => { onSelectBike(vehicle); setVehicleSearch(`${vehicle.registration} • ${vehicle.model}`); setVehiclePickerOpen(false); }} style={[styles.vehicleDropdownRow, vehicle.id === selectedBike.id && styles.vehicleDropdownSelected]}><View style={styles.vehicleDropdownIcon}>{vehicle.type === "car" ? <Ionicons name="car-sport" size={18} color={colors.blue} /> : <MaterialCommunityIcons name="motorbike" size={19} color={colors.blue} />}</View><View style={{ flex: 1 }}><Text style={styles.vehicleDropdownPlate}>{vehicle.registration}</Text><Text style={styles.vehicleDropdownModel}>{vehicle.model}</Text></View><View style={[styles.tableStatusDot, { backgroundColor: vehicle.status === "online" ? colors.green : colors.orange }]} /></Pressable>) : <Text style={styles.noVehicleText}>No vehicle found</Text>}</View>}</View>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const openApproval = notification => {
      ...
      if (active && data.type === "customer_approval" && typeof data.email === "string" && /^\d{6}$/.test(String(data.code || ""))) {
        setApplicant(current => ({ ...current, email: data.email }));
        setApprovalCode(String(data.code));
        setPhase("approval-notice");
      }
    };
    ...
    Notifications.getLastNotificationResponseAsync().then(response => {
      if (response) openApproval(response.notification);
    }).catch(() => {});
    return () => { active = false; responseSubscription.remove(); foregroundSubscription.remove(); };
  }, []);
  ...
  const showApprovalCode = useCallback(code => {
    setApprovalCode(code);
    setPhase("approval-notice");
    Alert.alert("Your account has been approved", `Your account has been approved. Your one-time verification code is ${code}. Do not share or expose this OTP.`, [{ text: "View code" }]);
  }, []);
  const backToLogin = useCallback(() => setPhase("auth"), []);
  return <SafeAreaProvider><AppUpdateNotice />{phase === "auth" && <AuthScreen pendingEmail={applicant?.email} approvedEmail={approvedEmail} onAuthenticated={authenticate} onPendingApproval={data => { setApplicant(data); if (data.otpCode) showApprovalCode(data.otpCode); else setPhase("pending"); }} />}{phase === "pending" && <PendingApproval applicant={applicant} onApproved={showApprovalCode} onEnterCode={() => setPhase("otp")} onBackToLogin={backToLogin} />}{phase === "approval-notice" && <ApprovalNotification applicant={applicant} code={approvalCode} onEnterCode={() => setPhase("otp")} onBack={() => setPhase("auth")} />}{phase === "otp" && <OtpVerification applicant={applicant} notificationCode={approvalCode} initialGate={false} onVerified={() => { if (applicant) setApprovedEmail(applicant.email); setApplicant(null); setApprovalCode(""); Notifications.clearLastNotificationResponseAsync().catch(() => {}); setPhase("auth"); }} onBack={applicant ? () => setPhase("pending") : undefined} />}{phase === "permissions" && <PermissionGate onComplete={() => setPhase("gps")} />}{phase === "gps" && <GpsLaunch name={displayName} returning={loginCount > 0} onComplete={() => { setLoginCount(count => count + 1); setPhase("app"); }} />}{phase === "app" && session && <CustomerApp session={session} onLogout={logout} />}</SafeAreaProvider>;
}
...
const styles = StyleSheet.create({
  ...
  approvalPage: { flex: 1, backgroundColor: colors.blueDark, paddingHorizontal: 25, paddingTop: 54, overflow: "hidden" }, approvalGlow: { position: "absolute", width: 330, height: 330, borderRadius: 165, backgroundColor: "rgba(9,105,218,.22)", top: -120, right: -120 }, approvalIcon: { width: 90, height: 90, borderRadius: 30, backgroundColor: "#FFF4DF", alignItems: "center", justifyContent: "center", alignSelf: "center", marginTop: 55 }, approvalTitle: { color: colors.white, fontSize: 25, fontWeight: "900", textAlign: "center", marginTop: 22 }, approvalText: { color: "#C8D8EA", fontSize: 13, lineHeight: 20, textAlign: "center", marginTop: 10 }, approvalWarning: { color: "#FFE2A9", fontSize: 12, lineHeight: 18, textAlign: "center", marginTop: 12 }, approvalSteps: { backgroundColor: "rgba(255,255,255,.08)", borderRadius: 18, padding: 17, marginTop: 28 }, approvalStep: { flexDirection: "row", alignItems: "center", gap: 11 }, approvalStepText: { color: colors.white, fontSize: 12, fontWeight: "700" }, approvalLine: { width: 2, height: 18, backgroundColor: "rgba(255,255,255,.16)", marginLeft: 10, marginVertical: 4 }, approvalButton: { minHeight: 50, borderRadius: 14, backgroundColor: colors.blue, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 9, marginTop: 24 }, approvalNoticeCard: { marginTop: 86, backgroundColor: colors.white, borderRadius: 22, padding: 22, alignItems: "center", shadowColor: "#000", shadowOpacity: .2, shadowRadius: 18, elevation: 10 }, approvalNoticeIcon: { width: 62, height: 62, borderRadius: 20, backgroundColor: colors.blue, alignItems: "center", justifyContent: "center" }, approvalNoticeTitle: { color: colors.ink, fontSize: 21, fontWeight: "900", textAlign: "center", marginTop: 16 }, approvalNoticeText: { color: colors.gray, fontSize: 13, textAlign: "center", marginTop: 10 }, approvalCode: { alignSelf: "stretch", backgroundColor: colors.bluePale, borderRadius: 14, paddingVertical: 15, marginTop: 15, alignItems: "center" }, approvalCodeText: { color: colors.blueDark, fontSize: 29, fontWeight: "900", letterSpacing: 6, marginLeft: 6 }, approvalNoticeHint: { color: colors.gray, fontSize: 11, lineHeight: 17, textAlign: "center", marginTop: 13 },
