...
function AuthScreen({ onAuthenticated, onPendingApproval, pendingEmail, approvedEmail }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      onAuthenticated({ ...session, loginPassword: password });
    } catch (error) {
      if (error instanceof ApiError && (error.status === 423 || error.code === "ACCOUNT_LOCKED")) Alert.alert("Account locked", "Four failed login attempts were detected. Contact your Jixels administrator to have the account opened.");
...
function ReportsScreen({ profile, selectedBike, vehicles, onSelectBike, onGenerate, onRefresh, isOnline, loginPassword }) {
  ...
  const openReport = async () => {
    if (reportAccessBlocked) return Alert.alert("Account blocked", "Report access is blocked after too many wrong password attempts. Contact Jixels admin.");
    setPassword("");
    setPasswordPromptVisible(true);
  };
  const verifyReportPassword = async () => {
    if (!loginPassword) return Alert.alert("Password check unavailable", "Sign in again before opening protected reports.");
...
const styles = StyleSheet.create({
  ...
  profileHero: { alignItems: "center", paddingTop: 24, paddingBottom: 18, marginTop: 8 }, avatar: { width: 76, height: 76, borderRadius: 25, backgroundColor: colors.blueDark, alignItems: "center", justifyContent: "center" }, avatarText: { color: colors.white, fontSize: 23, fontWeight: "900" }, profileName: { color: colors.ink, fontSize: 21, fontWeight: "900", marginTop: 10 }, profileId: { color: colors.gray, fontSize: 10, marginTop: 3 }, profileRow: { flexDirection: "row", gap: 11, alignItems: "center", paddingVertical: 11, borderBottomWidth: 1, borderBottomColor: colors.line }, profileRowIcon: { width: 36, height: 36, borderRadius: 12, backgroundColor: colors.bluePale, alignItems: "center", justifyContent: "center" }, profileValue: { color: colors.ink, fontSize: 12, fontWeight: "700", marginTop: 3 }, settingsSection: { backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, borderRadius: 18, padding: 15 }, settingsTitle: { color: colors.ink, fontSize: 17, fontWeight: "900" }, settingsCopy: { color: colors.gray, fontSize: 10, lineHeight: 15, marginTop: 4, marginBottom: 16 }, readonlyField: { minHeight: 57, borderWidth: 1, borderColor: colors.line, borderRadius: 14, backgroundColor: colors.surface, paddingHorizontal: 14, marginBottom: 15, flexDirection: "row", alignItems: "center", gap: 11 },
