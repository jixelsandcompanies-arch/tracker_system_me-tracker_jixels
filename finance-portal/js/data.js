(function () {
  ...
  async function registerFinanceUser({ name, email, phone, password }) { const result = await request("/auth/v1/signup", { method: "POST", body: JSON.stringify({ email, password, data: { full_name: name, phone } }) }); if (!result?.user) throw new Error("Finance registration could not be completed. Please try again."); if (!result.access_token) throw new Error("Check your email to confirm the account. An administrator must then approve Finance access."); const profile = await financeProfile(result.user.id, result.access_token); await hydrate(result.access_token); return { id: result.user.id, name: profile.full_name || name, email: result.user.email, phone: profile.phone || phone, role: profile.role, accessToken: result.access_token }; }
