...
import { hasSupabaseConfig, invokeApi, subscribeToTable } from "../lib/data";
...
export default function AgentAccountsView({ directory = false }) {
  ...
  useEffect(() => {
    load();
    const unsubscribe = subscribeToTable("profiles", load);
  ...
  const pageCopy = directory ? "Approved and rejected Finance and Agent accounts. Delete an account only when its access and linked workspace data must be removed." : "Pending Customer, Finance, and Agent accounts require an administrator decision. Customer approval sends the secure in-app code.";
