...
function downloadCsv(filename, records = []) {
  const url = URL.createObjectURL(
    new Blob([csv(records)], { type: "text/csv" }),
  );
  ...
  async function save(event) {
    ...
    const payload = Object.fromEntries(
      Object.keys(emptyCustomer).map((key) => [
        key,
        form[key] || (key === "status" ? "active" : null),
      ]),
    );
    const result = await updateRecord("customers", customer.id, payload);
    setSaving(false);
    if (result.error) return setMessage(result.error.message);
    recordAudit({
      action: "updated customer",
      resource: "Customers",
      detail: payload.full_name,
    });
    onSaved();
    onClose();
  }
      ...
      >
        <div className="detail-heading">
          <div>
            ...
            <h2>{`Update ${customer.full_name}`}</h2>
          </div>
        ...
        <form className="customer-record-form" onSubmit={save}>
          <div className="customer-form-grid">
            ...
            <label>
              Tracker number
              <input
                value={form.tracker_number || ""}
                onChange={(e) => set("tracker_number", e.target.value)}
              />
            </label>
            <label>
              Status
...
const customerState = (status) =>
  status === "pending"
    ? "pending"
    : ["suspended", "inactive"].includes(status)
      ? "rejected"
      : "approved";
const customerStateLabel = (status) =>
  customerState(status)[0].toUpperCase() + customerState(status).slice(1);
...
function CustomerDetails({ customer, onClose, onEdit }) {
  const details = [
    ...
    ["National ID / Passport", customer.national_id],
  ...
  return (
    <div className="detail-backdrop" onClick={onClose}>
      <aside
        className="detail-drawer workflow-drawer customer-details-drawer"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="detail-heading">
          <div>
            ...
            <p>Approved customer record</p>
          </div>
          <button className="icon-btn" onClick={onClose}>
            <X size={18} />
          </button>
        </div>
        <div className="detail-fields customer-detail-fields">
...
export default function CustomersView({ approvalMode = false }) {
  ...
  const visible = customers.filter(
    (customer) =>
      ...
      JSON.stringify(customer).toLowerCase().includes(search.toLowerCase()),
  );
  async function changeStatus(customer, status) {
    if (status === "active") {
      const result = await invokeApi("/v1/admin/screening/approve", {
        customerId: customer.id,
      });
      if (result.error) return setMessage(result.error.message);
      recordAudit({
        action: "approved customer and issued OTP",
        resource: "Customer Accounts",
        detail: customer.full_name,
      });
      setMessage(result.data?.message || "Customer approved and notified.");
      await load();
      return;
    }
    const result = await updateRecord("customers", customer.id, { status });
    if (result.error) return setMessage(result.error.message);
    recordAudit({
      action: "rejected customer account",
      resource: "Customers",
      detail: customer.full_name,
    });
    await load();
  ...
  async function removeCustomer() {
    const result = await invokeApi(
      `/v1/admin/users/${encodeURIComponent(pendingDelete.id)}`,
      null,
      "DELETE",
    );
    if (result.error) return setMessage(result.error.message);
    recordAudit({
      action: "deleted customer account and linked records",
      resource: "Customers",
      detail: pendingDelete.full_name,
    });
  ...
  async function removeCustomers(ids) {
    if (
      !ids.length ||
      !window.confirm(
        `Permanently delete ${ids.length} customer account${ids.length === 1 ? "" : "s"} and all linked records?`,
      )
    )
      return;
    for (const id of ids) {
    ...
    recordAudit({
      action: "deleted customer accounts and linked records",
      resource: "Customers",
      detail: `${ids.length} record(s)`,
    });
    setSelectedIds([]);
    await load();
  }
  async function importCsv(event) {
    ...
    if (
      rows[0]?.map((value) => value.toLowerCase()).join(",") !==
      fields.join(",")
    )
      setMessage(`Template headers must be: ${fields.join(", ")}.`);
    else {
      const records = rows
        .slice(1)
        .map((row) =>
        ...
        .map(({ customer_code, created_at, updated_at, ...record }) => ({
          ...record,
          ...(customer_code ? { customer_code } : {}),
          ...(created_at ? { created_at } : {}),
          ...(updated_at ? { updated_at } : {}),
        }));
      const result = await importRecords("customers", records);
      setMessage(
        result.error
          ? result.error.message
          : `${records.length} customers imported.`,
      );
      ...
      <>
        <section className="module-layout">
          <article className="panel module-summary">
            <div className="module-summary-icon">
              <Gauge size={22} />
            </div>
            <span>Total customers</span>
            <strong>{visible.length}</strong>
            <small>
              <Wifi size={13} /> Approved customer records
            </small>
          </article>
          <article className="panel module-table">
            <div className="panel-heading customer-heading">
              <div>
                <h2>Customers</h2>
                <p>
                  Approved customer records with contact, tracker, and update
                  controls.
                </p>
              </div>
              <div className="customer-top-actions">
                <button
                  className="button secondary"
                  onClick={() => downloadCsv("customers-template.csv")}
                >
                ...
                >
                  <ClipboardList size={15} /> Import CSV
                </button>
                <button
                  className="button secondary"
                  onClick={() => downloadCsv("jixels-customers.csv", visible)}
                >
            ...
            <div className="directory-filters">
              ...
              <button
                className="button secondary"
                onClick={() => removeCustomers(selectedIds)}
              >
                Delete selected
              </button>
              <button
                ...
                onClick={() =>
                  removeCustomers(visible.map((customer) => customer.id))
                }
              >
                Delete all
              </button>
                  ...
                  {visible.map((customer) => (
                    <tr key={customer.id}>
                      ...
                      <td>
                        ...
                        <small>{customer.customer_code || "Code pending"}</small>
                      </td>
                      <td>
                        {customer.phone || "—"}
  ...
  return (
    <>
        ...
        <article className="panel module-table">
          <div className="panel-heading customer-heading">
            <div>
              ...
              <p>
                Approved customer accounts. Pending registrations are reviewed
                in Screening.
              </p>
            </div>
            <div className="customer-top-actions">
      ...
      {pendingDelete && (
        ...
        >
          <aside
            className="detail-drawer confirm-drawer"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="detail-heading">
              <div>
                <span className="eyebrow">DELETE CUSTOMER ACCOUNT</span>
                <h2>{pendingDelete.full_name}</h2>
              </div>
              <button
                className="icon-btn"
                onClick={() => setPendingDelete(undefined)}
            ...
            <p>
              This permanently deletes this customer account and its linked
              workspace records. This action cannot be undone.
            </p>
            <div className="detail-actions">
              <button
                className="button secondary"
              ...
              <button className="button danger" onClick={removeCustomer}>
