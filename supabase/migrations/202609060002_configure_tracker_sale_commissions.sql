...
declare
  ...
  finance_rules jsonb := '{}'::jsonb;
  sale_commission numeric(12,2) := 0;
  monthly_customer_commission numeric(12,2) := 0;
begin
  ...
  select data into finance_rules
  from public.finance_settings
  where id = 'default';
  sale_commission := greatest(coalesce(nullif(finance_rules ->> 'saleCommission', '')::numeric, 0), 0);
  monthly_customer_commission := greatest(coalesce(nullif(finance_rules ->> 'monthlyCustomerCommission', '')::numeric, 0), 0);
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'monthlyCustomerCommission', monthly_customer_commission,
